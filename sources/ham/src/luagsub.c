...
static void add_s(MatchState* ms, const char* s, const char* e,
                  const char* news, size_t l)
{
  size_t i;
  for (i = 0; i < l; i++) {
    if (news[i] != L_ESC) {
...
int str_gsub(BUFFER* buff, const char* src, const char* p, const char* repl,
             int max_s)
{
  int anchor;
  int n = 0;
