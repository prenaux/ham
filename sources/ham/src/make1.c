...
static void make1bind(TARGET *t, int warn) {
  ...
  if (warn) {
    ...
    fputs("\n", stdout);
    puts(
      "  Anything that depends on it *will not be updated* when it changes or is");
    puts(
      "  modified. This also implies that your outputs will only be built if they");
    puts(
      "  do not already exist. Hint: You're probably missing a DEPENDS or equivalent.");
    fputs("\n", stdout);
  }
