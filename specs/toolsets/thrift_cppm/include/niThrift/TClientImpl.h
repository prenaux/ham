...
protected:
  ...
  std::shared_ptr<TMultiplexedProtocol> _multiplexProtocol;
...
public:
  ...
  virtual tBool __stdcall Connect() niOverride {
    try {
      ...
      _multiplexProtocol =
          std::make_shared<TMultiplexedProtocol>(_writeProtocol, _serviceName.c_str());

      _client = std::make_shared<TCLIENT>(_readProtocol, _multiplexProtocol);
